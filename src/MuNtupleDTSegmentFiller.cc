  ...
  MuNtupleBaseFiller(config, tree, label), m_tag(tag), m_nullVecF()
{
  ...
  // m_nullVecF(0) = MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL;

}

MuNtupleDTSegmentFiller::~MuNtupleDTSegmentFiller() 
{ 

};

void MuNtupleDTSegmentFiller::initialize()
{
...
void MuNtupleDTSegmentFiller::clear()
{
...
void MuNtupleDTSegmentFiller::fill(const edm::Event & ev)
{
	  ...
	  for (; segment4D != segment4DEnd; ++segment4D)
	    {
	      ...
	      float xPosLocSL[2] = { MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL,
				     MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL };
	      bool hasPptSL[2] = { false, false };
	      float xPosLocMidPlane = MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL;
		  ...
		  for(const auto & wire : wireVector)
		    {
		      ...
		      float theExpPos = MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL;
		      float theExpPosCh = MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL;
		      int theExpWire = MuNtupleBaseFiller::DEFAULT_INT_VAL_POS;
...
void MuNtupleDTSegmentFiller::fillPhi(const DTChamberRecSegment2D* phiSeg, const GeomDet* chamb)
{
  ...
  for(const auto & recHit : recHits)
    {
      ...
      auto phaseTag = m_tag == Tag::PH1 ?
	              MuNtupleConfig::PhaseTag::PH1 :
	              MuNtupleConfig::PhaseTag::PH2;
...
void MuNtupleDTSegmentFiller::fillZ(const DTSLRecSegment2D* zSeg, const GeomDet* chamb)
{
...
void MuNtupleDTSegmentFiller::fillPhiEmpty()
{
		
  m_seg2D_phi_nHits.push_back(0);
  m_seg2D_phi_t0.push_back(MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL);
  m_seg2D_phi_vDrift.push_back(MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL);
  m_seg2D_phi_normChi2.push_back(MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL_POS);
...
void MuNtupleDTSegmentFiller::fillZEmpty()
{
		
  m_seg2D_z_nHits.push_back(0);
  m_seg2D_z_normChi2.push_back(MuNtupleBaseFiller::DEFAULT_DOUBLE_VAL_POS);
